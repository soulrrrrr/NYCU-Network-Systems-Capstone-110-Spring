...
static void print_packet_info(const u_char *packet, const struct pcap_pkthdr *packet_header) {
    static int d = 0;
    static struct in_addr tunnels[256]; // 256 is a random number I choosed
    ...
    */
    ...
    if (outer_ip->ip_p == 0x2F) // GRE number from wikipedia
        printf("GRE\n");
    else
        printf("Not GRE\n");

    /* GRE header base: 34 */
    void *gre = (void *)(packet+34);
    ...
    struct ether_header *inner_eth = (struct ether_header *)(packet+38);

    /* Source MAC */ 
    printf("Inner source MAC: ");
    for (int i = 0; i < 6; i++) {
        printf("%02x", (u_int)inner_eth->ether_shost[i]);
        if (i != 5)
            printf(":");
        else
            printf("\n");
    ...
    for (int i = 0; i < 6; i++) {
        printf("%02x", (u_int)inner_eth->ether_dhost[i]);
        if (i != 5)
            printf(":");
        else
            printf("\n");
    ...
    for(int i = 0; i < tunnel_num; i++) {
        if ((u_int)tunnels[i].s_addr == (u_int)outer_ip->ip_src.s_addr) {
            flag = false;
            break;
        } 
    }
    ...
    if (flag) {
        ...
        sprintf(
            command, 
            "ip link add GRETAP_%d type gretap remote %s local %s", 
            tunnel_num,
            remote,
            local);
        system(command);
        ...
        sprintf(filter_add, " && host not %s", remote);
        ...
        /* finish message */
        printf("GRE tunnel %d builded.\n", tunnel_num);
        tunnels[tunnel_num++] = outer_ip->ip_src;
    }
